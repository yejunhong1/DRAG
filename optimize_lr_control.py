...
eps_greedy    = 0
epsilon       = 0.3
write_sum     = 1000              # key/interval for writing summary data to file
...
def main():
    for pt in range(1,3):
        for nm in range(3,4):
            runs(pt, nm)
            print( "Finish pt=" + str(pt) + ", nm=" + str(nm) + " at " + time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()) )
    return 0

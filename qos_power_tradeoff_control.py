...
def runs(beta0, m, shift=0, r=1, funname=''):
    ...
    rewards, sum_powers, switch_powers, qos_costs  = dqn_bsa( AR, ac_ref=4, write_sum=write_sum, net_scale=1, funname=funname, beta0=beta0 )
...
if __name__ == '__main__':
    m = 10
    betas = [100]#range(10, 110, 10)
    for si in range(1): # si<=8
        for r in range(10): #[0]
            shift = int(si*1000)
            for beta0 in betas: #[0]
                funname = 'qos_power_tradeoff_m_' +str(m) +  '_beta_' + str(beta0) + '_s_' +str(shift) + '_r_' + str(r)

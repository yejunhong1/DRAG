...
ac_ref       = 4                # initial action refinement method
...
def main():
    ...
    data_file = './traffic_pattern/AR_'+patterns[pattern_type-1]+'_n_10_d_' + str(days) + '_nm_' + noise_para[noise_method] # 'Simulated_arrival_rates_no_noise_' + str(days)   #
    samples   = sio.loadmat( data_file )
    arrival_rates = samples['AR']

    num_sbs, steps = arrival_rates.shape
    steps = 5000                                 # The number of steps (can freely modify)
    ...
    agent = DDPG( his_ar_size, ar_size, action_size, TAUt, is_batch_norm, write_sum, net_size_scale, beta0=thr_coe )
    ...
    for i in range( num_his_ar, steps ):
        ...
        if ac_ref<=3:
            ...
            if random.uniform(0, 1) < agent.decay(i, 0, 3, num_his_ar, steps*0.75, 2):
        ...
        reward, _, _,_, throughput = agent.env.find_reward( real_load, action, prev_action )   #
        #print("in main, throughput: "+str(throughput))
        ...
        cerror = 1
        aerror = 1
        ac_train_times = min(16, max(1, int(i/500)) ) #if i<3840 else 0
        ...
        if i%(48) == 0:
            mrt = np.mean( rewards[i-48:i] )
            if write_sum>0:
                print( " ------- i: %5d, arp-e: %1.5f, lm-e: %1.5f, a-e: %1.5f, c-e: %1.5f, d-reward: %1.5f \n" % ( i, arp_errors[-1], lm_errors[-1], a_errors[-1], c_errors[-1], mrt ) )
            else:
                print( " ------- i: %5d, mean reward: %1.5f \n" % ( i, mrt ) )

    #agent.close_all()      # this will write network parameters into .txt files


    writetext( actions,     directory+'actions',    1 )
    writetext( actions_o,   directory+'actions_o',  1 )
    writetext( pred_ars,    directory+'pred_ar',    1 )
    writetext( rewards,     directory+'rewards'       )
    writetext( mean_reward, directory+'mean_rewards'  )
    writetext( arp_errors,  directory+'arp_errors', 1 )
    writetext( lm_errors,   directory+'lm_errors',  1 )
    writetext( c_errors,    directory+'c_errors'      )
    writetext( a_errors,    directory+'a_errors'      )
    writetext( real_loads,  directory+'real_loads', 1 )
    writetext( pred_loads,  directory+'pred_loads', 1 )
    writetext( throughputs, directory+'throughputs'   )

    matdict = {}
    matdict['actions']   = actions
    matdict['rewards']   = rewards
    matdict['actions_o'] = actions_o
    matdict['c_errors']    = c_errors
    matdict['a_errors']    = a_errors
    matdict['throughputs'] = throughputs

    sio.savemat('bsa_data.mat', matdict)

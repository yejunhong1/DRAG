...
class CriticNet_bn_3:
    ...
    def __init__(self, state_size, action_size, TAU = 0.001, write_sum = 0, net_size_scale=1 ):
        ...
        with self.g.as_default():
            ...
            self.A3, self.para3, self.bn_para3 = DNN.nn_bn_layer( self.A2,    self.hidden_dims[1], 
                self.hidden_dims[2], "Layer_3", self.activations[2], self.is_training, self.sess )
            self.Q_value, self.out_paras = DNN.nn_layer( self.A3, self.hidden_dims[2], self.output_size, "Output", self.activations[3] )

            self.parameters = [self.para1[0], self.W2, self.W2_action, self.para3[0] ]
            self.parameters.extend( self.out_paras )
            self.bn_paras = [ self.bn_para1[0], self.BN2, self.bn_para3[0] ]
            ...
            self.t_A3, self.t_para3, self.t_bn_para3 = DNN.nn_bn_layer( self.t_A2,    self.hidden_dims[1], 
                self.hidden_dims[2], "T_Layer_3", self.activations[2], self.is_training, self.sess, self.bn_paras[2] )

            self.t_Q_value, self.t_out_paras = DNN.nn_layer( self.t_A3, self.hidden_dims[2], self.output_size, "T_Output", self.activations[3] )

            self.t_parameters = [self.t_para1[0], self.t_W2, self.t_W2_action, self.t_para3[0] ]
            self.t_parameters.extend( self.t_out_paras )
            self.t_bn_paras = [ self.t_bn_para1[0], self.t_BN2, self.t_bn_para3[0] ]
    ...
    def train_critic( self, state_batch, action_batch, q_obj, learning_rate=0.0001 ):
        self.sess.run( self.train_op, feed_dict={ self.state: state_batch, self.t_state: state_batch, 
            self.action: action_batch, self.t_action: action_batch, self.learning_rate: learning_rate, self.Q_obj: q_obj, self.is_training: True } )
        cerror = 1
        if (self.write_sum >0 ) and (self.counter%self.write_sum == 0):
            cerror, summary = self.sess.run( [ self.cost, self.merged ], feed_dict={ self.state: state_batch, 
                self.t_state: state_batch, self.action: action_batch, self.t_action: action_batch, 
                self.Q_obj: q_obj, self.learning_rate: learning_rate, self.is_training: False } )

...
class LoadMapNet:
    ...
    def __init__(self, ar_size, action_size, load_size, write_sum = 0, net_size_scale=1 ):
        ...
        with self.g.as_default():   #
            ...
            self.mapped_load, self.lm_layer_parls = DNN.multilayer_perceptron( self.ar_action_in, 
                self.ar_action_size, self.n_hidden, self.load_size, self.activations )
    ...
    def evaluate_load_map(self, arrival_rate, action):
        ...
        mapped_load = self.sess.run(self.mapped_load, feed_dict={self.ar_action_in:ar_action_in})
        mapped_load = np.clip( mapped_load, 0, 0.998 )
        return mapped_load
        
        
    def train_load_map(self, ar_action_in, real_load_in, learning_rate=0.0001):
        self.sess.run(self.optimizer, feed_dict={ self.ar_action_in: ar_action_in, 
            self.real_load: real_load_in, self.learning_rate: learning_rate})        
        
        error = 1
        if (self.write_sum >0 ) and (self.counter%self.write_sum == 0):
            error, summary = self.sess.run([self.cost, self.merged], 
                feed_dict={ self.ar_action_in: ar_action_in, self.real_load: real_load_in})
    ...
    def close_all( self ):
        ...
        #DNN.write_var( self.sess, self.lm_layer_parls, self.lm_parameters_name, filename="lm_var" )
